...
import { z } from 'zod';
import { ArrowRight, Check, Send, ShieldCheck, Smartphone, KeyRound, AlertTriangle, CheckCircle2, Lock, RotateCw } from 'lucide-react';
import { useAuth } from '@/legacy/contexts/AuthContext';
import { fetchApi } from '@/legacy/services/apiClient';

const mobileIdentifierSchema = z.string().trim().regex(/^\+?[0-9 ()-]{7,20}$/, 'Enter your registered mobile number.');
...
export function ForgotPasswordForm() {
  ...
  const handleRequestResetOtp = async (e?: React.FormEvent) => {
    ...
    setErrorMsg('');
    setSuccessMsg('');
    const parsed = mobileIdentifierSchema.safeParse(identifier);
    if (!parsed.success) {
    ...
    setLoading(true);
    try {
      const result = await fetchApi<{ expires_in: number; message?: string }>('/member-auth/forgot-password/', {
        method: 'POST',
        body: JSON.stringify({ identifier: parsed.data }),
        skipAuthRefresh: true,
      });
      setSuccessMsg(result.message || 'Reset code sent to your registered mobile number.');
        ...
        <section className="p-6 sm:p-9">
          <div className="mb-6">
            ...
            <p className="mt-1.5 text-xs text-slate-600">
              {step === 1 ? 'Enter your registered mobile number to receive a verification code.' : 'Enter the code you received along with your new password.'}
            </p>
          </div>
          ...
          {step === 1 && (
            <form onSubmit={handleRequestResetOtp} className="space-y-4">
              ...
              >
                {loading ? 'Sending Code...' : <><Send className="w-4 h-4" /> Send OTP by Mobile SMS</>}
              </button>
            </form>
          )}
...
export function VerifyOtpForm() {
  ...
  const request = async () => {
    const parsed = mobileIdentifierSchema.safeParse(getValues('identifier'));
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(217,179,108,0.18),_transparent_30%),linear-gradient(145deg,_#fffdf9_0%,_#f7f0e8_100%)] px-4 pb-12 pt-28 sm:px-6 sm:pt-32">
      <div className="mx-auto grid w-full max-w-5xl overflow-hidden rounded-[2rem] border border-[#e11d48]/12 bg-white shadow-[0_24px_70px_rgba(43,16,29,0.13)] md:grid-cols-[0.82fr_1.18fr]">
        ...
        <section className="p-6 sm:p-9"><div className="mb-7"><span className="inline-flex items-center gap-2 rounded-full border border-[#d9b36c]/35 bg-[#fff8e9] px-3 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.14em] text-[#6f451c]"><ShieldCheck className="h-4 w-4 text-[#e11d48]" /> Mobile verification</span><h2 className="mt-4 font-display text-2xl font-bold tracking-tight text-[#1e293b] sm:text-3xl">{contactMode ? 'Verify your mobile number' : 'Sign in with a mobile code'}</h2><p className="mt-2 text-sm leading-relaxed text-slate-600">{contactMode ? 'Enter your registered mobile number and confirm the code below.' : 'Use your registered mobile number to receive a secure code.'}</p></div>
          <form onSubmit={verify} noValidate className="space-y-5"><label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-[#68535d]">Registered mobile number</span><div className="relative"><Smartphone className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[#e11d48]" /><input autoComplete="tel" defaultValue={user?.mobile_number || ''} {...register('identifier', { required: true })} placeholder="Enter your registered mobile number" className="w-full rounded-xl border border-[#1e293b]/12 bg-[#f9f6f1] py-3 pl-10 pr-4 text-sm text-[#1e293b] outline-none transition placeholder:text-slate-400 focus:border-[#e11d48]/45 focus:bg-white focus:ring-4 focus:ring-[#e11d48]/10" /></div></label><button type="button" onClick={request} className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-[#e11d48]/25 bg-[#f6e9ee] px-4 py-3 text-sm font-bold text-[#e11d48] transition hover:border-[#e11d48]/45 hover:bg-[#f0dfe6]"><Send className="h-4 w-4" /> Send mobile verification code</button><div className="mt-3 text-center">{resendCooldown > 0 ? (<span className="inline-flex items-center gap-1.5 text-xs font-semibold text-slate-400"><RotateCw className="h-3.5 w-3.5" /> Resend available in {resendCooldown}s</span>) : (<button type="button" onClick={request} className="inline-flex items-center gap-1.5 text-xs font-bold text-[#e11d48] transition hover:underline"><RotateCw className="h-3.5 w-3.5" /> Didn't receive? Resend code</button>)}</div><div className="relative py-1 text-center"><span className="relative z-10 bg-white px-3 text-[11px] font-bold uppercase tracking-[0.12em] text-slate-400">Then enter your code</span><span className="absolute left-0 right-0 top-1/2 h-px bg-[#1e293b]/10" /></div><label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[0.12em] text-[#68535d]">Verification code</span><div className="relative"><Smartphone className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-[#e11d48]" /><input inputMode="numeric" autoComplete="one-time-code" {...register('code', { required: true })} placeholder="Enter the code you received" className="w-full rounded-xl border border-[#1e293b]/12 bg-[#f9f6f1] py-3 pl-10 pr-4 text-sm text-[#1e293b] outline-none transition placeholder:text-slate-400 focus:border-[#e11d48]/45 focus:bg-white focus:ring-4 focus:ring-[#e11d48]/10" /></div></label><button disabled={isSubmitting} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#e11d48] px-4 py-3.5 text-sm font-bold text-white shadow-md transition hover:bg-[#6f2c41] disabled:cursor-not-allowed disabled:opacity-60">{isSubmitting ? 'Verifying…' : <>{contactMode ? 'Verify mobile number' : 'Verify and sign in'} <ArrowRight className="h-4 w-4" /></>}</button></form>
