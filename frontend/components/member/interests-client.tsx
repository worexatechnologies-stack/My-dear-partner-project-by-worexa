...
export function InterestsClient({ mode }: { mode: InterestMode }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const pendingIncoming = incomingItems.filter((item) => item.status === 'PENDING');
      const acceptedMatches = [
        ...incomingItems.filter((item) => item.status === 'ACCEPTED'),
        ...sentItems.filter((item) => item.status === 'ACCEPTED'),
      ];
      const declinedIncoming = incomingItems.filter((item) => item.status === 'DECLINED');
      const sentRequests = sentItems.filter(
                      ...
                      {isOutgoing && item.status === 'PENDING' && (
                        withdrawConfirmingId === item.id ? (
                          <div className="rounded-xl border border-rose-200 bg-rose-50 p-3">
                            <p className="mb-2 text-[11px] font-semibold leading-relaxed text-rose-800">
                              Unsend this request? The member will no longer be able to accept it.
                            </p>
                            <div className="flex gap-2">
                              <button
                                onClick={() => setWithdrawConfirmingId(null)}
                                disabled={isBusy}
                                className="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-bold text-slate-600 transition-colors hover:bg-slate-50 disabled:opacity-50"
                              >
                                Keep Request
                              </button>
                              <button
                              ...
                              >
                                {isBusy ? 'Unsending...' : 'Unsend Request'}
                              </button>
                            </div>
                          </div>
                        ) : (
                          <button
                            ...
                            disabled={isBusy}
                            className="w-full rounded-xl border border-rose-200 bg-rose-50 px-3 py-2.5 text-xs font-bold text-rose-700 transition-colors hover:bg-rose-100 disabled:opacity-50"
                          >
                            Unsend Request
                          </button>
                        )
                      ...
                      {item.status === 'ACCEPTED' && (
                        <>
                          ...
                          {matchRemovalConfirmingId === item.id ? (
                            <div className="rounded-xl border border-rose-200 bg-rose-50 p-3">
                              <p className="mb-2 text-[11px] font-semibold leading-relaxed text-rose-800">
                                Remove this match? Both of you will lose chat and contact access. Existing messages stay private but cannot be opened unless a new interest is accepted.
                              </p>
                              <div className="flex gap-2">
                                <button
